...
BEGIN
  ...
  IF NEW.title IS DISTINCT FROM OLD.title OR NEW.slug IS NULL OR NEW.slug = '' THEN
...
-- Update existing pages with slugs
UPDATE pages SET slug = generate_slug(title) WHERE slug IS NULL OR slug = '';